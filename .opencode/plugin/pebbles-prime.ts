...
export const PebblesPrimePlugin: Plugin = async ({ $ }) => {
  ...
  return {
    ...
    tool: {
      ...
      peb_update: tool({
        description: "Update a peb. Provide a JSON string with the fields to update (e.g., '{\"status\":\"fixed\"}')",
        args: {
          id: tool.schema.string().describe("The peb ID to update (e.g., peb-xxxx)"),
          data: tool.schema.string().describe("JSON string with update data (e.g., '{\"status\":\"fixed\"}')"),
        },
        async execute(args) {
          const proc = spawn(['peb', 'update', args.id, args.data], {
            stdout: 'pipe',
          });
          const result = await new Response(proc.stdout).text();
          await proc.exited;
      ...
      peb_query: tool({
        ...
        args: {
          ...
          fields: tool.schema.string().optional().describe("Specific fields to output (e.g., 'id,title')"),
        },
        async execute(args) {
          const cmdArgs: string[] = ['peb', 'query'];
          if (args.fields) {
            cmdArgs.push('--fields', args.fields);
          }
          if (args.filters) {

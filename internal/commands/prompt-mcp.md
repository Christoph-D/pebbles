...
Use the `peb_new` tool with:
- `title`: Short description of the peb
- `content`: Markdown description of the peb
- `type` (optional): One of `bug`, `feature`, `epic`, `task` (default: `bug`)
- `blocked_by` (optional): Array of peb IDs that block this peb

Example: Create a bug peb
```
Call peb_new with:
- title: "Fix login bug"
...
Example: Create a peb blocked by another
```
Call peb_new with:
- title: "Fix UI"
- content: "..."
- blocked_by: ["{{.PebbleIDPattern}}"]
```

### Read a peb

Use the `peb_read` tool with:
- `id`: The peb ID to read (e.g., `{{.PebbleIDPattern}}`)

Returns full peb data as JSON.

### Update a peb

Use the `peb_update` tool with:
- `id`: The peb ID to update (e.g., `{{.PebbleIDPattern}}`)
- `data`: JSON string with update data
...
Update blocked-by dependencies:
Call peb_update with:
- id: "{{.PebbleIDPattern}}"
- data: '{"blocked-by":["{{.PebbleIDPattern2}}","{{.PebbleIDPattern3}}"]}'
```

### Query pebs

Use the `peb_query` tool with:
- `filters` (optional): Array of filter strings
- `fields` (optional): Specific fields to output (e.g., `"id,title"`)

Examples:
```
List all pebs:
Call peb_query with no filters
...
Call peb_query with:
- filters: ["blocked-by:{{.PebbleIDPattern}}"]

Combine filters (implicit AND):
Call peb_query with:
- filters: ["status:new", "type:bug"]
...
1. Use `peb_query` with `filters: ["status:new"]` to find work

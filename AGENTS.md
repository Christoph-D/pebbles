# AGENTS.md - Code Style & Development Guide

## Build & Test

The project uses a Makefile for common development tasks:

- `make` - Build the binary to `bin/peb` (default target)
- `make test` - Run all tests with verbose output
- `make test-coverage` - Run tests with coverage report (generates
  coverage.html)
- `make clean` - Remove build artifacts and coverage files
- `make install` - Install the binary to GOPATH/bin
- `make fmt` - Format code with go fmt
- `make vet` - Run go vet for static analysis
- `make mod-tidy` - Tidy go.mod dependencies
- `make deps` - Download dependencies
- `make lint-ts` - Typecheck the generated pi extension (`internal/config/data/pebbles-pi.ts`) with `tsc` (requires npm; installs deps into `tools/tslint/node_modules`)
- `make help` - Show all available targets

## Running the Binary

After building with `make`, the binary is available at `bin/peb`:

```bash
# Run directly
./bin/peb <command>

# Or install to PATH for easier access
make install
# Now you can use: peb <command>
```

## Module Structure

`pebbles` is a task tracking CLI tool that manages "pebs" (tasks/bugs/features)
as markdown files with YAML frontmatter.

- `cmd/peb/` - CLI app entry point (urfave/cli)
- `internal/commands/` - Command implementations (init, new, read, update,
  query, prime)
- `internal/config/` - Configuration loading from `.pebbles/config.toml`
- `internal/peb/` - Core data model, validation, file I/O (YAML frontmatter)
- `internal/store/` - Storage layer, ID generation, lookups

## Auto-Generated Files (DO NOT EDIT)

The `.opencode/` and `.pi/` directories are **auto-generated by `peb`**
(e.g. via `peb prime`). Never modify their contents by hand — your
changes will be overwritten on the next generation. If something needs to
change, update the source data/templates that produce them instead.

## Go Code Style

### Structs & Interfaces

- Use dependency injection via concrete types unless you have multiple
  implementations
- Don't create interfaces unless you have multiple implementations or need to
  mock for tests
- Keep structs simple, focus on single responsibility
- Use pointer receivers for methods that modify state
- Use value receivers for methods that don't modify state

## CLI Command Changes

After making changes to any CLI command, you MUST check if the following places also need to be updated:

- **MCP tools** - `internal/commands/data/pebbles.ts` - Ensure MCP tool definitions reflect the updated command
- **The prompt** - `internal/commands/data/prompt.md` - Update documentation and examples if command behavior or usage changes
- **MCP prompt** - `internal/commands/data/prompt-mcp.md` - Update MCP-specific documentation
- **Other Markdown files** - `README.md`, `AGENTS.md` - Review and update documentation files that reference the command
